...
def fill_boxes():
    ...
    for item in functions.numeric_generator_iter(functions.read("C:\\Behrad Todo\\todo_list.txt"), 1, " -- "):
        if "".join(item.split("-")[1:]).replace(" ", "") != "":
            todo_list.insert("end", item)
    for item in functions.numeric_generator_iter(functions.read("C:\\Behrad Todo\\comp_list.txt"), 1, " -- "):
        if "".join(item.split("-")[1:]).replace(" ", "") != "":
